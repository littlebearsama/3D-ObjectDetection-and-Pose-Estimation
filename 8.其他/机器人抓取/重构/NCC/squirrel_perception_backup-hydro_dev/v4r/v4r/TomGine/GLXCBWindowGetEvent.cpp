...
void GLWindow::GetEventBlocking(Event &event)
{
  do{
    ...
    /* Wait for event */
    xcb_generic_event_t *xcb_event = xcb_wait_for_event(connection);
    if(!xcb_event)
    {
    ...
    case XCB_KEY_PRESS:{
      xcb_key_press_event_t *kp = (xcb_key_press_event_t *)xcb_event;
      KeySym *ks = XGetKeyboardMapping(dpy, kp->detail, 1, &keysyms_per_keycode_return);
      MapKey(ks[0], event.input);
      XFree(ks);
      event.type = TMGL_Press;
      break;
    }
    case XCB_KEY_RELEASE:{
      xcb_key_release_event_t *kr = (xcb_key_release_event_t *)xcb_event;
      KeySym *ks = XGetKeyboardMapping(dpy, kr->detail, 1, &keysyms_per_keycode_return);
      MapKey(ks[0], event.input);
      XFree(ks);
    ...
    case XCB_BUTTON_PRESS:{
      ...
      event.type = TMGL_Press;
      MapMouse(bp->detail, event.input);
      event.motion.x = bp->event_x;
      event.motion.y = bp->event_y;
      break;
    }
    case XCB_BUTTON_RELEASE:{
      xcb_button_press_event_t *br = (xcb_button_press_event_t *)xcb_event;
      event.type = TMGL_Release;
      MapMouse(br->detail, event.input);
      event.motion.x = br->event_x;
      event.motion.y = br->event_y;
      break;
    }
