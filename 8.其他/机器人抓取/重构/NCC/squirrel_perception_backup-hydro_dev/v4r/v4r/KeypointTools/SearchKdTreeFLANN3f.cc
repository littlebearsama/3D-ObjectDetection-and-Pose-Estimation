 ...
 */
SearchKdTreeFLANN3f::SearchKdTreeFLANN3f(const Parameter &p)
...
SearchKdTreeFLANN3f::~SearchKdTreeFLANN3f()
{
  clean();
}
 ...
 */
void SearchKdTreeFLANN3f::convertPointsToArray(const std::vector<Eigen::Vector3f> &points)
{
  if (points.size()==0)
    return;
 ...
 * setInputPoints
 */
void SearchKdTreeFLANN3f::setInputPoints(const std::vector<Eigen::Vector3f> &points)
{
  clean();
  dim = 3;
 ...
 * nearestKSearch
 */
void SearchKdTreeFLANN3f::nearestKSearch(const Eigen::Vector3f &pt, int k, std::vector<int> &k_indices, std::vector<float> &k_distances)
{
 ...
 * radiusSearch
 */
void SearchKdTreeFLANN3f::radiusSearch(const Eigen::Vector3f &pt, double radius, std::vector<int> &k_indices, std::vector<float> &k_sqr_dists, unsigned int max_nn)
{
  if (!data || isnan(pt[0]) || isnan(pt[1]) || isnan(pt[2]))
  {
    k_indices.clear();
 ...
 * nearestKSearch
 */
void SearchKdTreeFLANN3f::nearestKSearch(int idx, int k, std::vector<int> &k_indices, std::vector<float> &k_distances)
{
 ...
 * radiusSearch
 */
void SearchKdTreeFLANN3f::radiusSearch(int idx, double radius, std::vector<int> &k_indices, std::vector<float> &k_sqr_dists, unsigned int max_nn)
{
  if (idx>=total_nr_points || !data)
  {
    k_indices.clear();
 ...
 * clean
 */
void SearchKdTreeFLANN3f::clean()
{
  if (flann_index)
    delete flann_index;
